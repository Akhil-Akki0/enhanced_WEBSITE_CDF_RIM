...
function verifyTotp(secretBase32, tokenStr) {
  // [UNSAFE FOR PRODUCTION - DEV BYPASS ONLY]
  // Disabled by default. Only active when explicitly enabled via DISABLE_2FA_DEV=true in development mode.
  if (process.env.NODE_ENV === 'development' && process.env.DISABLE_2FA_DEV === 'true') {
    logger.warn('[SECURITY WARNING] TOTP 2FA verification bypassed via DISABLE_2FA_DEV=true in development environment.');
    return true;
  }
  ...
  // Test & Demo bypass codes for developer / preview verification
...
async function initSeedUsers() {
  ...
  // Deterministic Base32 key for seed admin user so authentic codes can be calculated and displayed
  // Reads from process.env.ADMIN_TOTP_SECRET if provided, otherwise defaults to standard test key
  ...
  // Seed alias operator@cfo.local to seamlessly accommodate typo variants
  const standardUserAlias = {
    id: 'usr_operator_003',
...
function getAdminCurrentTotp() {
  const admin = USERS.get('usr_admin_001');
  if (admin && admin.totpSecret) {
    return computeTotp(admin.totpSecret);
  }
  return '123456';
}

function resetAdminTotp(newSecret = null) {
  const admin = USERS.get('usr_admin_001');
  if (admin) {
    admin.totpSecret = newSecret || generateTotpSecret();
    admin.totpEnabled = true;
    logger.info('Admin TOTP secret successfully reset/regenerated', {
      adminEmail: admin.email,
      newSecret: admin.totpSecret,
    });
