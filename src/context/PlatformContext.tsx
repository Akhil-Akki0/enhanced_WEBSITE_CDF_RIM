...
export const PlatformProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Pure Light Theme (enforces bright, clean, attractive design without dark spots)
  const [themeMode, setThemeMode] = useState<ThemeMode>('light');

  const toggleThemeMode = () => {
    sound.playTick();
    setThemeMode((prev) => (prev === 'light' ? 'light' : 'light'));
  };

  useEffect(() => {
    // Enforce light theme across document root
