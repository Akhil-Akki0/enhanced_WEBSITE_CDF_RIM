...
export const CfdViewport: React.FC<CfdViewportProps> = ({
  ...
  // Initialize or replenish particles (enhanced particle density for performance & aerospace visuals)
  const initParticles = useCallback((count = 350) => {
    const list: Particle[] = [];
    for (let i = 0; i < count; i++) {
      list.push({
        x: -1.4 + Math.random() * 0.6,
      ...
      if (showStreamlines) {
        ...
        particlesRef.current.forEach((p) => {
          ...
          if (p.history.length > 2) {
            ...
            // Color based on local flow velocity and Mach number
            let strokeColor = 'rgba(37, 99, 235, 0.65)';
            let dotColor = '#2563EB';

            if (field.normVelocity > 1.35) {
              // High speed / supersonic acceleration
              strokeColor = 'rgba(249, 115, 22, 0.85)';
              dotColor = '#FB923C';
            } else if (field.normVelocity > 1.15) {
              // Suction acceleration
              strokeColor = 'rgba(6, 182, 212, 0.8)';
              dotColor = '#57DFFE';
            } else if (field.normVelocity < 0.65) {
            ...
            ctx.lineWidth = field.normVelocity > 1.15 ? 1.6 : 1.1;
            ctx.stroke();
            ...
            ctx.beginPath();
            ctx.arc(head[0], head[1], field.normVelocity > 1.15 ? 1.8 : 1.3, 0, Math.PI * 2);
            ctx.fill();
          }
        });
      }
